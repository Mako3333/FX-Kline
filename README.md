...
フェーズ1で実装されたコアロジックは完全に独立しており、MCPサーバーとして簡単に再利用できます。

### 予定内容

- mcp_server/ ディレクトリに MCP Tool実装
- fetch_ohlc: 単一取得ツール
- fetch_ohlc_batch: 並列バッチ取得ツール
- list_available_pairs: 利用可能ペア一覧
- Claude Desktop統合設定ファイル生成スクリプト

### MCPサーバー化による利点

- Claude DesktopやCline等のAIツールから直接呼び出し可能
- 自然言語でのデータ取得リクエスト(AI側で解釈)
- ワークフロー自動化(複数ツール連携)
